...
export function HelpPage({ pageKey }: { pageKey: HelpPageKey }) {
    ...
    return (
        <div className="min-h-screen bg-primary text-primary">
            <header className="border-b border-secondary bg-primary"><div className="mx-auto flex h-16 max-w-5xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8"><a href="/" className="inline-flex min-h-11 items-center text-lg font-semibold text-primary">{STOREFRONT.brand.name}</a><UuiButton href="/shop" color="tertiary" size="lg">Termékek</UuiButton></div></header>
            <main className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8 lg:py-20"><Badge color="brand">{page.eyebrow}</Badge><h1 className="mt-5 max-w-3xl text-balance text-display-md font-semibold tracking-tight text-primary">{page.title}</h1><p className="mt-5 max-w-3xl text-pretty text-lg leading-8 text-secondary">{page.intro}</p><div className="mt-10 grid gap-4 sm:grid-cols-2">{page.sections.map((section, index) => <section key={section.title} className={`min-w-0 rounded-2xl bg-secondary p-5 ring-1 ring-secondary sm:p-6 ${index === 0 ? "sm:col-span-2" : ""}`}><h2 className="text-lg font-semibold text-primary">{section.title}</h2><p className="mt-2 text-md leading-7 text-secondary">{section.copy}</p></section>)}</div>
                {pageKey === "cookies" && <section className="mt-8 rounded-2xl bg-primary p-5 shadow-sm ring-1 ring-secondary sm:p-6"><h2 className="text-lg font-semibold text-primary">Választásod</h2><div className="mt-5 grid gap-4"><Checkbox className="min-h-11" label="Szükséges sütik" hint="Mindig bekapcsolva" isSelected isDisabled /><Checkbox className="min-h-11" label="Elemzési sütik" hint="A bolt javításához" isSelected={analytics} onChange={setAnalytics} /><Checkbox className="min-h-11" label="Hirdetési mérés" hint="Meta, Google és TikTok" isSelected={marketing} onChange={setMarketing} /></div><UuiButton className="mt-6" size="lg" onPress={() => { saveConsent({ analytics, marketing }); setSaved(true); }}>Beállítások mentése</UuiButton><p className="mt-3 text-sm text-secondary" role="status" aria-live="polite">{saved ? "A beállításokat elmentettük." : "A mérés engedély nélkül kikapcsolva marad."}</p></section>}
                {pageKey === "contact" && STOREFRONT.contact.email && <UuiButton href={`mailto:${STOREFRONT.contact.email}`} className="mt-8" size="lg" iconTrailing={ArrowRight}>E-mail írása</UuiButton>}
            </main>
        </div>
    );
}
