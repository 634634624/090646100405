...
export function CheckoutPage() {
    ...
    const beginCheckout = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ lines }) });
            const result = await response.json() as { checkoutUrl?: string; error?: string };
            if (!response.ok || !result.checkoutUrl) throw new Error(result.error || "A pénztár nem indítható.");
            window.location.assign(result.checkoutUrl);
        } catch (checkoutError) {
            setError(checkoutError instanceof Error ? checkoutError.message : "A pénztár nem indítható. A kosár megmaradt.");
            setLoading(false);
        }
    };
    return (
        <div className="min-h-screen bg-primary text-primary">
            <header className="border-b border-secondary bg-primary"><div className="mx-auto flex h-16 max-w-5xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8"><a href="/" className="inline-flex min-h-11 items-center text-lg font-semibold text-primary">{STOREFRONT.brand.name}</a><UuiButton href="/cart" color="tertiary" size="lg">Vissza a kosárhoz</UuiButton></div></header>
