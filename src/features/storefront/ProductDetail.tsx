        ...
        <div className="min-h-screen bg-primary text-primary">
            <header className="border-b border-secondary bg-primary">
                <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
                    <a href="/" className="inline-flex min-h-11 shrink-0 items-center text-lg font-semibold tracking-tight text-primary outline-brand focus-visible:outline-2 focus-visible:outline-offset-2">{STOREFRONT.brand.name}</a>
                    <div className="flex items-center gap-1">
                        ...
                        <UuiButton href="/cart" color="tertiary" size="lg" iconLeading={ShoppingBag03} className="hidden sm:inline-flex">Kosár ({cartCount})</UuiButton>
                        <UuiButton aria-label={mobileNavOpen ? "Navigáció bezárása" : "Navigáció megnyitása"} aria-expanded={mobileNavOpen} aria-controls="product-mobile-navigation" color="tertiary" size="lg" iconLeading={mobileNavOpen ? X : Menu01} onPress={() => setMobileNavOpen((open) => !open)} className="md:hidden" />
                    </div>
                </div>
                {mobileNavOpen && <nav id="product-mobile-navigation" aria-label="Mobil navigáció" className="border-t border-secondary px-4 py-3 md:hidden"><div className="mx-auto flex max-w-7xl flex-col gap-1"><UuiButton href="/shop" color="tertiary" size="lg" className="justify-start">Termékek</UuiButton><UuiButton href="/kategoriak/muszaki" color="tertiary" size="lg" className="justify-start">Műszaki</UuiButton><UuiButton href="/kategoriak/haztartas" color="tertiary" size="lg" className="justify-start">Háztartás</UuiButton><UuiButton href="/cart" color="tertiary" size="lg" iconLeading={ShoppingBag03} className="justify-start">Kosár ({cartCount})</UuiButton></div></nav>}
            </header>
                ...
                <section className="bg-secondary">
                    <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 py-8 sm:px-6 md:grid-cols-[minmax(0,1.08fr)_minmax(20rem,0.92fr)] md:items-start md:gap-12 md:py-14 lg:px-8">
                        ...
                        <div className="md:sticky md:top-8">
                            <UuiButton href="/shop#termekek" color="link-gray" size="lg" className="min-h-11">Vissza a termékekhez</UuiButton>
                            ...
                            <div className="mt-8">
                                ...
                                <div className="mt-3 flex items-center gap-2" role="group" aria-label="Mennyiség">
                                    <UuiButton aria-label="Mennyiség csökkentése" size="lg" color="secondary" iconLeading={Minus} isDisabled={quantity <= 1} onPress={() => setQuantity((value) => Math.max(1, value - 1))} />
                                    <span className="min-w-10 text-center text-md font-semibold text-primary" aria-live="polite">{quantity}</span>
                                    <UuiButton aria-label="Mennyiség növelése" size="lg" color="secondary" iconLeading={Plus} isDisabled={quantity >= Math.max(1, product.stock)} onPress={() => setQuantity((value) => Math.min(Math.max(1, product.stock), value + 1))} />
                                </div>
                            </div>
            ...
            <footer className="bg-primary"><div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-8 text-sm text-tertiary sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8"><p>© 2026 {STOREFRONT.legal.companyName}</p><a className="inline-flex min-h-11 items-center font-medium text-secondary hover:text-primary" href="/shop">Minden termék</a></div></footer>
        </div>
    );
}
