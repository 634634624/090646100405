...
test("builds a current UCP cart request without secrets", () => {
    const request = checkoutRequest([{ variantId: DEMO_VARIANT_IDS[1], quantity: 3 }]);
    assert.equal(request.params.name, "create_cart");
    assert.deepEqual(request.params.arguments.cart.line_items, [{ quantity: 3, item: { id: DEMO_VARIANT_IDS[1] } }]);
    assert.equal(JSON.stringify(request).includes("token"), false);
});

test("uses capability-specific profiles for catalog and cart", () => {
    const catalog = catalogRequest();
    const checkout = checkoutRequest([{ variantId: DEMO_VARIANT_IDS[0], quantity: 1 }]);
